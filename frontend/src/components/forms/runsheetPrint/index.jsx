...
export default function RunsheetPrint() {
  ...
  const [fromDate, setFromDate] = useState(getDateForInput(get30DaysBeforeDate()));
  ...
  const [runsheetList, setRunsheetList] = useState([]);
  ...
  const fetchData = async (query) => {
    setLoading(true);
    try {
      const res = await fetch(serverUrl + 'runsheet?' + query, {
        credentials: 'include',
      });
      const json = await res.json();
      if (res.status == 500) {
        message.error('Internal Server Error Occured: ' + json.err);
      } else if (res.status != 200) {
        message.error('Error Occured: ', json.msg);
      } else {
        if (json.data.length > 0) message.success('Runsheets Loaded');
        else message.warning('Runsheets Not Found');
        setRunsheetList((p) => [...json.data]);
        const data = json.data.map((r,i)=>{
            return {
                sno:i+1,
                runsheetNo:r?.runsheetNumber,
                date:getFormttedDate(r?.date),
                empName:r?.employee?.name,
                totalDockets:r?.dockets?.lenght
            }            
        })
        setExcelData(p=>[...data])
      }
  ...
  useEffect(() => {
    let query = '';
    if (currBranch?._id) {
      query += `bid=${currBranch._id}&`;
    }
    if (fromDate && toDate) {
      query += `fromDate=${fromDate}&toDate=${toDate}&`;
    }
    if (runsheetFrom) {
      query += `runsheetFrom=${runsheetFrom}&`;
    }
    if(runsheetTo){
        query += `runsheetTo=${runsheetTo}`;
    }
    fetchData(query);
  }, [currBranch, search]);
  ...
  const resetForm = () => {
    setRunsheetFrom('');
    setFromDate(getDateForInput(get30DaysBeforeDate()));
    ...
    <>
      ...
      <div className={style.formContainer}>
        ...
        <div>
          ...
          <input
            type="date"
            value={getDateForInput(fromDate)}
            onInput={(e) => setFromDate(e.target.value)}
          />
          <label htmlFor="">To</label>
          <input
            type="date"
            value={getDateForInput(toDate)}
            onInput={(e) => setToDate(e.target.value)}
          />

          <label htmlFor="">Runsheet From</label>
          <input
            type="text"
            value={runsheetFrom}
            placeholder='Runsheet From'
            onInput={(e) => setRunsheetFrom(e.target.value)}
          />
          <label htmlFor="">To</label>
          <input
            type="text"
            value={runsheetTo}
            placeholder='Runsheet To'
            onInput={(e) => setRunsheetTo(e.target.value)}
          />
        </div>
      </div>
      ...
      <div className={style.actions}>
        ...
        <ExportToExcel fileName={'Runsheets'} apiData={excelData}/>
                ...
                runsheetList?.map((r) => {
                  return (
                    <tr>
                      <td>
                        <FaPrint
                          onClick={(e) => downloadRunsheet(r?._id)}
                          style={{ color: 'blueviolet', fontSize: '18px' }}
                        />
                      </td>
