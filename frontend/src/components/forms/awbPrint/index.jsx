...
export default function AwbPrint() {
  ...
  const docketsHandler = (e) => {
    setDocket((p) => {
      ...
      if (e.target.value == '') obj.dockets = [];
      else
        obj.dockets = e.target.value
          .split(',')
          .map((item) => item.trim())
          .filter((item) => item !== '');
      return obj;
    ...
    try {
      ...
      if (logo != null)
        endpoint =
          ...
          logo +
          '&dockets=' +
          docket.dockets.toString();
      else
        endpoint =
          'awb?branch=' +
          currBranch?._id +
          '&sticker=true&dockets=' +
          docket.dockets.toString();

      const response = await useDownloader(endpoint);
  ...
  return (
    <>
      ...
      <div className={style.actions}>
        <button className={style.buttonChk} onClick={(e) => printDoc(true)}>
          <BsPrinter /> Print
        </button>
        <button className={style.buttonChk} onClick={(e) => printDoc(false)}>
          <BsPrinter /> Print without logo
        </button>
        <button className={style.buttonChk} onClick={(e) => printDoc(null)}>
          <BsPrinter /> Sticker Print
        </button>
