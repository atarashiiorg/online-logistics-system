...
//     const [err, loading, manifests, setManifests] = useGetData(`manifest${currBranch?._id?"?bid="+currBranch._id:""}`, [currBranch, reload])
...
//         <>
//         {
//             isSubmitting||loading?<Loading/>:null
//         }
...
const TableRow = (props) => {
  const { dockets, select, date, message, allChecked } = props;
  return (
    <>
      {dockets.map((d) => {
          ...
          if (e.target.checked) {
            select((p) => {
              ...
              if (idx > -1) {
                ...
                return [...p, { docket: id, rcDate: date, message }];
              }
            });
            setIsChecked(true);
          } else {
          ...
          if (allChecked) {
            select((p) => {
              ...
              if (idx > -1) {
                ...
                return [...p, { docket: d?._id, rcDate: date, message }];
              }
            });
          } else {
            select((p) => p.filter((docket) => docket?.docket !== d?._id));
            setIsChecked(false);
          }
        }, [allChecked]);
...
export default function ReceiveAwbNo() {
  ...
  const [reload, setReload] = useState(false);
  const [err, loading, manifests, setManifests] = useGetData(`manifest${currBranch?._id ? '?bid=' + currBranch._id : ''}`, [currBranch, reload]);
  const [receiveAllCheck, setReceiveAllCheck] = useState(false);
  const [selectedDockets, setSelectedDockets] = useState([]);
  const [rcDate, setRcDate] = useState(getDateForInput());
  const [msg, setMessage] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);
  ...
  const handleReceive = async () => {
    if (manifests.length <= 0) {
      message.warning('No dockets are here to receive.');
      return;
    }
    if (selectedDockets.length <= 0) {
      message.warning('Please mark dockets to receive.');
      return;
    }
    if (rcDate === '') {
  ...
  return (
    <>
      ...
      <div className={style.formContainer}>
        <p>Receive AwbNo Details</p>
        <div>
          <label htmlFor="rcDate">RcDate</label>
          <input
            id="rcDate"
            type="date"
            value={getDateForInput(rcDate)}
            ...
            <thead>
              <tr>
                <th>
                  <input
                    type="checkbox"
                    checked={receiveAllCheck}
                    onChange={(e) => setReceiveAllCheck(e.target.checked)}
                  /> All
                </th>
                <th>Docket No</th>
                <th>Manifest No</th>
              ...
              {manifests.length > 0 ? (
                manifests.map((m) => (
                  <TableRow
                    key={m._id}
                    {...m}
                    allChecked={receiveAllCheck}
                    select={setSelectedDockets}
                    date={rcDate}
                    message={msg}
                  />
                ))
      ...
      <div className={style.actions}>
        <button className={style.buttonChk} onClick={handleReceive}>
          <FaCheck /> Receive
        </button>
        <button className={style.buttonExp}>
          <BsFiletypeXls /> Export
        </button>
        <button className={style.buttonRef} onClick={() => setReload((prev) => !prev)}>
